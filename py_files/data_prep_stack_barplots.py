...
def make_tax_dfs(taxon_list, feature_table):
    ...
    # columns are categories within each taxonomy level found in data, indexes are the samples
...
def fill_tax_dfs(kingdom_df,phylum_df,class_df,order_df,family_df,genus_df,species_df,feature_table,taxonomy_red):
    ...
    for sample, row in feature_table.iterrows(): 
        ...
        for featid in row.index:
            # iterate over each feature id (sequence) in sample and add prevalence to taxonomy df
            if featid in taxonomy_red.index:
                tax_list = taxonomy_red.loc[featid]
                # gets a list of the corresponding kingdom, phylum, etc for the feature id
...
def prepare_data_stacked_barplots(feature_table, taxonomy_table):
    ...
    kingdom_df, phylum_df, class_df, order_df, family_df, genus_df, species_df = fill_tax_dfs(kingdom_df_0, phylum_df_0, 
                                                                                              class_df_0, order_df_0, 
                                                                                              family_df_0, genus_df_0, 
                                                                                              species_df_0, feature_table, taxonomy_red)
    return kingdom_df, phylum_df, class_df, order_df, family_df, genus_df, species_df
