...
from data_prep_stack_barplots import reduce_taxonomy_df, make_taxon_list, make_tax_dfs, fill_tax_dfs
from alpha_div_gen import alpha_diversity_plot
from beta_div_gen import bray_beta_diversity_clean, unifrac_beta_diversity_clean
...
def test_fill_tax_dfs():
    ...
    taxon_list = [
        [
            'bacteria', 'archaea'], [
            'bacteriodetes', 'firmicutes', 'proteobacteria'], ['bacilli'], ['bacteroidia'], [
                'Bacteroidaceae', 'Neisseriaceae'], ['Neisseria'], [
                    'species1', None]]
    ...
    kingdom_result, phylum_result, class_result, order_result, family_result, genus_result, species_result = make_tax_dfs(
        taxon_list, feat_tab_df)
    kingdom_filled, phylum_filled, class_filled, order_filled, family_filled, genus_filled, species_filled = fill_tax_dfs(
        kingdom_result, phylum_result, class_result, order_result, family_result, genus_result, species_result, feat_tab_df, tax_df)
    kingdom_ans = pd.DataFrame(
        data={
            '': [
                'samp1', 'samp2', 'samp3'], 'bacteria': [
                0.5, 0.5, 0.0], 'archaea': [
                    0.5, 0.5, 1.0]})
    kingdom_ans.set_index('', inplace=True)
    assert kingdom_filled.equals(
        kingdom_ans), 'taxonomy kingdom dataframe is not being properly filled in fill_tax_dfs'
    phylum_ans = pd.DataFrame(
        data={
            '': [
                'samp1', 'samp2', 'samp3'], 'bacteriodetes': [
                0.0, 0.25, 0.0], 'firmicutes': [
                    0.5, 0.25, 0.0], 'proteobacteria': [
                        0.5, 0.5, 1.0]})
    phylum_ans.set_index('', inplace=True)
    assert phylum_filled.equals(
        phylum_ans), 'taxonomy phylum dataframe is not being properly filled in fill_tax_dfs'
    class_ans = pd.DataFrame(
        data={
            '': [
                'samp1', 'samp2', 'samp3'], 'bacilli': [
                1.0, 1.0, 1.0]})
    class_ans.set_index('', inplace=True)
    assert class_filled.equals(
        class_ans), 'taxonomy class dataframe is not being properly filled in fill_tax_dfs'
    order_ans = pd.DataFrame(
        data={
            '': [
                'samp1', 'samp2', 'samp3'], 'bacteroidia': [
                1.0, 1.0, 1.0]})
    order_ans.set_index('', inplace=True)
    assert order_filled.equals(
        order_ans), 'taxonomy order dataframe is not being properly filled in fill_tax_dfs'
    family_ans = pd.DataFrame(
        data={
            '': [
                'samp1', 'samp2', 'samp3'], 'Bacteroidaceae': [
                0.0, 0.25, 0.0], 'Neisseriaceae': [
                    1.0, 0.75, 1.0]})
    family_ans.set_index('', inplace=True)
    assert family_filled.equals(
        family_ans), 'taxonomy family dataframe is not being properly filled in fill_tax_dfs'
    genus_ans = pd.DataFrame(
        data={
            '': [
                'samp1', 'samp2', 'samp3'], 'Neisseria': [
                1.0, 1.0, 1.0]})
    genus_ans.set_index('', inplace=True)
    assert genus_filled.equals(
        genus_ans), 'taxonomy genus dataframe is not being properly filled in fill_tax_dfs'
    species_ans = pd.DataFrame(
        data={
            '': [
                'samp1', 'samp2', 'samp3'], 'species1': [
                0.0, 0.25, 0.0], None: [
                    1.0, 0.75, 1.0]})
    species_ans.set_index('', inplace=True)
    assert species_filled.equals(
        species_ans), 'taxonomy species dataframe is not being properly filled in fill_tax_dfs'
        ...
        data={
            '': [
                ...
                0.0, 0.0, 0.0], 'archaea': [
                    0.0, 0.0, 0.0]})
    kingdom_ans.set_index('', inplace=True)
    assert kingdom_result.equals(
        kingdom_ans), 'taxonomy kingdom dataframe is not being properly initialized in make_tax_dfs'
    phylum_ans = pd.DataFrame(
        data={
            '': [
                'samp1', 'samp2', 'samp3'], 'bacteriodetes': [
                0.0, 0.0, 0.0], 'firmicutes': [
                    0.0, 0.0, 0.0], 'proteobacteria': [
                        0.0, 0.0, 0.0]})
    phylum_ans.set_index('', inplace=True)
    assert phylum_result.equals(
        phylum_ans), 'taxonomy phylum dataframe is not being properly initialized in make_tax_dfs'
    class_ans = pd.DataFrame(
        data={
            '': [
                'samp1', 'samp2', 'samp3'], 'bacilli': [
                0.0, 0.0, 0.0]})
    class_ans.set_index('', inplace=True)
    assert class_result.equals(
        class_ans), 'taxonomy class dataframe is not being properly initialized in make_tax_dfs'
    order_ans = pd.DataFrame(
        data={
            '': [
                'samp1', 'samp2', 'samp3'], 'bacteroidia': [
                0.0, 0.0, 0.0]})
    order_ans.set_index('', inplace=True)
    assert order_result.equals(
        order_ans), 'taxonomy order dataframe is not being properly initialized in make_tax_dfs'
    family_ans = pd.DataFrame(
        data={
            '': [
                'samp1', 'samp2', 'samp3'], 'Bacteroidaceae': [
                0.0, 0.0, 0.0], 'Neisseriaceae': [
                    0.0, 0.0, 0.0]})
    family_ans.set_index('', inplace=True)
    assert family_result.equals(
        family_ans), 'taxonomy family dataframe is not being properly initialized in make_tax_dfs'
    genus_ans = pd.DataFrame(
        data={
            '': [
                'samp1', 'samp2', 'samp3'], 'Neisseria': [
                0.0, 0.0, 0.0]})
    genus_ans.set_index('', inplace=True)
    assert genus_result.equals(
        genus_ans), 'taxonomy genus dataframe is not being properly initialized in make_tax_dfs'
    species_ans = pd.DataFrame(
        data={
            '': [
                'samp1', 'samp2', 'samp3'], 'species1': [
                0.0, 0.0, 0.0], None: [
                    0.0, 0.0, 0.0]})
    species_ans.set_index('', inplace=True)
    assert species_result.equals(
        species_ans), 'taxonomy species dataframe is not being properly initialized in make_tax_dfs'


def test_make_taxon_list():
    """test function"""
    test_d = {'Feature ID': ['feat1', 'feat2', 'feat3'],
              'kingom': ['bacteria', 'bacteria', 'archaea'],
              'phylum': ['bacteriodetes', 'firmicutes', 'proteobacteria'],
              'class': ['bacilli', 'bacilli', 'bacilli'],
              'order': ['bacteroidia', 'bacteroidia', 'bacteroidia'],
    ...
    test_df.set_index('Feature ID', inplace=True)
    test_df.head()
    result = make_taxon_list(test_df)
    ...
    test_d = {
        'col1': [
            ...
            1, 2, 3], 'Confidence': [
                4, 5, 6]}
    test_df = pd.DataFrame(data=test_d)
    result = reduce_taxonomy_df(test_df)
    d_red = {'col1': ['a', 'b', 'c']}
    answer = pd.DataFrame(d_red)
    assert answer.equals(
        result), 'taxonomy DataFrame is not being properly reduced'
    test_d = {
        'Taxon': [
            1, 2, 3], 'col2': [
            'a', 'b', 'c'], 'Confidence': [
...
def test_find_dropoff():
    ...
    assert isinstance(
        depth, int), 'depth must be an int, got ' + str(type(depth))
...
def test_get_feature_info():
    ...
    if min_feats <= 100:
        ...
        suggest = min_feats - 1
    assert isinstance(
        suggest, np.int64), 'suggestion is not an int, got ' + str(type(suggest))
...
def test_bray_beta_diversity_clean_1():
    ...
    assert isinstance(bray_metadata_to_plot,
                      pd.core.frame.DataFrame), "not returning df"
...
def test_unifrac_beta_diversity_clean_1():
    ...
    assert isinstance(unifrac_metadata_to_plot,
                      pd.core.frame.DataFrame), "not returning df"
...
def test_unifrac_beta_diversity_clean_3():
    ...
    bray_metadata_to_plot = bray_beta_diversity_clean('../data')
    unifrac_metadata_to_plot = unifrac_beta_diversity_clean('../data')
    assert unifrac_metadata_to_plot['body-site'].all(
    ) == bray_metadata_to_plot['body-site'].all()
...
def test_unifrac_beta_diversity_clean_4():
    ...
    false = bray_metadata_to_plot.iloc[1,
                                       9] == unifrac_metadata_to_plot.iloc[1, 9]
...
def test_alpha_diversity_plot_1():
    ...
    assert isinstance(alpha_metadata_to_plot,
                      pd.core.frame.DataFrame), "not returning df"
...
def test_alpha_diversity_plot_2():
    ...
    alpha_metadata_to_plot = alpha_diversity_plot('../data')
    if 'sample-id' and 'faith_diversity' and 'pielou_evenness' in alpha_metadata_to_plot.columns:
        result = 1
    else:
        result = 0
...
def test_alpha_diversity_plot_3():
    ...
    assert isinstance(
        result, np.float64), "values not populating for alpha metrics"
