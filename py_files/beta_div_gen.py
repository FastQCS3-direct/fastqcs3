...
def sample_cols(directory):
    """function fetches relevant columns names from metadata, omits barcode sequences/sample id"""
    # Load metadata
...
def bray_beta_diversity_clean(directory):
    """function generates final data frame for bray-curtis pcoa beta diversity plots"""
    ...
    bray = pd.read_csv(
        directory +
        '/bray-distance-matrix.tsv',
        sep='\t',
        index_col=0)
    bray_different_index = pd.read_csv(
        directory + '/bray-distance-matrix.tsv', sep='\t')
    ...
    bray_pcoa_results = bray_sample_id.join(
        bray_final_results, on='index', lsuffix='l', rsuffix='r')
    bray_pcoa_results = bray_pcoa_results.drop(
        labels=['indexl', 'indexr'], axis=1)
...
def unifrac_beta_diversity_clean(directory):
    """function generates final data frame for weighted unifrac pcoa beta diversity plots"""
    # Load and clean metadata
    metadata = pd.read_csv(directory + '/metadata.tsv', sep='\t')
    metadata = metadata.drop(metadata.index[0])
    metadata = metadata.reset_index(drop=True)
    ...
    unifrac = pd.read_csv(
        directory +
        '/unifrac_distance_matrix.tsv',
        sep='\t',
        index_col=0)
    unifrac_different_index = pd.read_csv(
        directory + '/unifrac_distance_matrix.tsv', sep='\t')
    ...
    unifrac_sample_id = unifrac_sample_id.rename(
        columns={'Unnamed: 0': 'sample-id'})
    unifrac_sample_id = unifrac_sample_id.reset_index()
    # Constructing final matrix with PCoA Results AND metadata
    unifrac_pcoa_results = unifrac_sample_id.join(
        unifrac_final_results, on='index', lsuffix='l', rsuffix='r')
    unifrac_pcoa_results = unifrac_pcoa_results.drop(
        labels=['indexl', 'indexr'], axis=1)
    unifrac_metadata_to_plot = metadata.merge(
        unifrac_pcoa_results, how='outer')
